	...
	<div class="table">
		<div class="crumbs">
			<el-breadcrumb separator="/">
				...
				<el-breadcrumb-item>教练推荐</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
		...
		<el-table :data="tableData" border stripe style="width:100%" v-loading="listLoading">
			...
			<el-table-column prop="isHot" label="是否热门">
				<template scope="scope">
					<span v-if="scope.row.isHot === false">否</span>
					<span v-else>是</span>
			    </template>
			</el-table-column>
		...
		<!--添加-->
		<div class="panel">
			<el-dialog v-model="addFormVisible" size="small" title="添加推荐教练">
				<el-form :inline="true" :model="recQuery" class="demo-form-inline">
					<el-form-item label="条件查询">
						<el-input v-model="recQuery.gym" placeholder="输入健身房名称或识别码搜索" style="width:300px"></el-input>
					</el-form-item>
					<el-form-item>
						<el-input v-model="recQuery.keyword" placeholder="输入教练姓名搜索"></el-input>
					</el-form-item>
					<el-form-item>
						<el-button type="primary" icon="search" @click="handleCoachList()">查询</el-button>
					</el-form-item>
				</el-form>
				<el-table :data="CoachList" border stripe style="width:100%;" v-loading="listLoading" :hidden="hidden">
					<el-table-column prop="serialNumber" label="教练ID"> </el-table-column>
...
<script>
	...
	export default {
		...
		methods: {  
			getTotal() {
				this.$axios({
						url: this.path + 'resource/coach/recommend/count',
						method: 'get',
			...
			getData() {
				...
				this.$axios({
						url: this.path + 'resource/coach/recommend/list',
						method: 'get',
						params: this.listQuery,
						transformRequest: [function(params) {
							let ret = ''
			...
			handleRecom(index,row) {
				this.$confirm('确认要推荐吗？', '提示', { type: 'warning' }).then(() => {
					this.$axios({
						url: this.path + 'resource/coach/recommend/' + row.id,
						method: 'get',
						headers: {
							'Content-Type': 'application/x-www-form-urlencoded',
							'Authorization':this.token,
			...
			handleDelete(index, row) {
				this.$confirm('确认要取消推荐吗？', '提示', { type: 'warning' }).then(() => {
					this.$axios({
						url: this.path + 'resource/coach/unrecommend/' + row.id,
						method: 'get',
						headers: {
							'Content-Type': 'application/x-www-form-urlencoded',
							'Authorization':this.token,
